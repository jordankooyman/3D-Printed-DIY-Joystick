...
// It is highly recommended to make a copy of this file for each joystick you build, as each will have different calibration values and button arrangements.

// Compile and upload this using Arduino IDE
//   Make sure to install the Adafruit ADS1115 module from Tools->Manage Libraries and 
//   the Joystick library from: https://github.com/MHeironimus/ArduinoJoystickLibrary, following the included tutorial)

// The button mapping and the hat switch processing will need to be modifed independently for each joystick that is wired manually (unless multiple are somehow wired identically)
//   To update how the button maping is done, modify the code at the very bottom in the JoystickButtons function
//   To determine which button is which ID, test each one using the Serial_Mode feature, either mode will work.
//     Edit the provided table above to show which id is which for your controller

// To calibrate the joystick movement axes:
//  1. Uncomment Serial_Mode (leave the value at 1 for the calibration data)
//  2. Upload code to joystick
//  3. Carefully move an axis to your desired deadzone bounds and enter the listed min/max values in the code below as prompted
//        For me, the play in the axis before the spring return starts to take affect
//        I recommend doing 1 axis at a time the first few times, and increasing the range slightly, and testing it until it feels right
//           Testing can be done either using the mode 2 on Serial_Mode or using the Setup USB Game Controllers system in Windows
//  4. After deadzone has been determined, push the axis as far as you want it to go in measured values and 
//      enter the listed min/max values in the code below as prompted
//  5. Repeat until all 4 axes have been calibrated - it should not require any windows calibration

// There should be no need to modify the JoystickAxes function
//   If you want to disable the deadzone, set the min and max values to be in the center of your range, with a difference of one
//       See the throttle deadzone for an example calculation
...
//                                   Depending on the tolerances of your joystick, typically 2 is the largest this should be
...
// The following is an example of how to disable the deadzone
#define Throttle_max_deadzone ((Throttle_max_value - Throttle_min_value) / 2 + 1) // Set to the max value of the desired deadzone
#define Throttle_min_deadzone (Throttle_max_deadzone - 1)                         // Set to the min value of the desired deadzone
...
//#define CLOCK_ENABLE_PIN2 - just connect to ground
#define SERIAL_IN_PIN2 5
#define LOAD_DATA_PIN2 7
#define CLOCK_PIN 15
//#define CLOCK_ENABLE_PIN - just connect to ground
