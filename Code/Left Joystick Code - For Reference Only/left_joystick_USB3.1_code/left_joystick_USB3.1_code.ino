...
// Initialize Joystick Library
  ...
  Joystick.setYAxisRange(min_Pitch, max_Pitch); //Pitch 8000-13000
  Joystick.setXAxisRange(min_Roll, max_Roll); //Roll 7900-15300
  Joystick.setZAxisRange(min_Yaw, max_Yaw); //Yaw  10400-14200
...
adc2 = ads.readADC_SingleEnded(2);  // Throttle  0-24850
int16_t adc3; // 16 bits ADC read of input A3
adc3 = ads.readADC_SingleEnded(3);  //X Roll 7900-15300
...
if (adc2 < deadzone_lower_Throttle) {
  adc2 += deadzone_range_Throttle + 1100;
}
else if (adc2 > deadzone_upper_Throttle) {
  adc2 -= deadzone_range_Throttle + 1100;
}
else {
  adc2 = ((0 + Throttle_offset - offset_Throttle));//deadzone_lower_Throttle + deadzone_range_Throttle + Throttle_trim;
}  
adc2 += offset_Throttle; 

if (adc3 < deadzone_lower_Roll) {
  adc3 += deadzone_range_Roll + 1500;
}
else if (adc3 > deadzone_upper_Roll) {
  adc3 -= deadzone_range_Roll + 500;
}
else {
  adc3 = ((0 + Roll_offset - offset_Roll));//deadzone_lower_Roll + deadzone_range_Roll + Roll_trim;
}   
adc3 += offset_Roll;

adc3 = ((adc3 - Roll_offset) * invert_Roll);
adc1 = ((adc1 - Pitch_offset) * invert_Pitch);
adc0 = ((adc0 - Yaw_offset) * invert_Yaw);
adc2 = ((adc2 - Throttle_offset) * invert_Throttle);

if (adc0 > 0) {adc0 = adc0 / 1.15;}//Yaw
if (adc1 > 0) {adc1 = adc1 / 1.0;}//Pitch
if (adc3 > 0) {adc3 = adc3 / 1.4;}//Roll
if (adc2 > 0) {adc2 = adc2 / 1.005;}//Throttle
